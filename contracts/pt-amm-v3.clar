;; Vort PT AMM V3 - Constant-product AMM for PT/sBTC
;; V3: Uses real testnet sBTC + fixes the sender/recipient bug from v1/v2
...
(define-public (swap-sbtc-for-pt (sbtc-in uint) (min-pt-out uint))
  (let (
    ...
    ;; Transfer real sBTC from user to pool
    (try! (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer sbtc-in sender (as-contract tx-sender) none))
...
(define-public (swap-pt-for-sbtc (pt-in uint) (min-sbtc-out uint))
  (let (
    ...
    ;; Transfer real sBTC from pool to user
    (try! (as-contract (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer sbtc-out tx-sender sender none)))
...
(define-public (add-liquidity (sbtc-amount uint) (pt-amount uint))
  (let (
    ...
    (try! (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer sbtc-amount sender (as-contract tx-sender) none))
