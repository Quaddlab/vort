;; Vort Tokenizer V2 - Core Minting and Redemption Engine
;; Deposit sBTC -> Mint equal PT + YT (1:1)
;; Redeem PT + YT -> Return sBTC (only after maturity)
;;
;; V2 changes: uses (as-contract ...) when calling pt-token/yt-token
;; mint and burn so that the tokenizer acts as the authorized-minter,
;; allowing ANY wallet to deposit, not just the deployer.
...
(define-public (redeem (amount uint))
  (let ((user tx-sender))
    ...
    ;; Return sBTC to user
    (try! (as-contract (contract-call? .sbtc-token transfer amount tx-sender user none)))
