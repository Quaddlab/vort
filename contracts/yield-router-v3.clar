;; Vort Yield Router V3 - Yield Accrual, Distribution & Zest Integration
;; V3: Uses real testnet sBTC + fixes accrue-yield to pull sBTC from caller first
...
;; Accrue yield - admin calls this to simulate Zest Protocol returns.
;; FIXED in V3: First pulls sBTC from the caller INTO this contract,
;; then distributes the treasury share from the contract's balance.
(define-public (accrue-yield (amount uint))
  (let (
    ...
    ;; Step 4: Transfer treasury share from contract to treasury address
    (try! (as-contract
      (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token
        transfer treasury-share tx-sender (var-get treasury-address) none)))
...
(define-public (claim-yield)
  (let (
    ...
    ;; Transfer real sBTC yield to user from this contract's balance
    (try! (as-contract
      (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token
        transfer claimable tx-sender caller none)))
